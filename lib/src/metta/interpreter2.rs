...
impl<'a, T: SpaceRef<'a>> InterpreterState<'a, T> {
    ...
    fn push(&mut self, atom: InterpretedAtom) {
        match atom_as_slice(&atom.0) {
            Some([op, ..]) if *op == RETURN_SYMBOL => {
                let InterpretedAtom(atom, _bindings) = atom;
                let [_, returned] = atom_into_array(atom).unwrap();
                if returned != EMPTY_SYMBOL {
                    self.finished.push(returned);
                }
            },
            _ => self.plan.push(atom),
        }
    }
}
...
fn return_empty() -> Atom {
    Atom::expr([RETURN_SYMBOL, EMPTY_SYMBOL])
}

fn error_atom(atom: Atom, err: String) -> Atom {
    Atom::expr([RETURN_SYMBOL, Atom::expr([Atom::sym("Error"), atom, Atom::sym(err)])])
}
...
fn evaluate_atom<'a, T: SpaceRef<'a>>(space: T, atom: Atom, bindings: Bindings) -> Vec<InterpretedAtom> {
    match atom {
        Atom::Expression(ref expr) if expr.children().len() > 0 => {
            // TODO: clone() could be eliminated if Grounded::execute() signature would return
            // NoReduce with specific atom. Otherwise we need to keep original atom to return
            // it with NoReduce result.
            let mut children = expr.clone().into_children();
            let op = children.remove(0);
            match op {
                Atom::Grounded(gnd) => execute_atom(atom, gnd, children, bindings),
...
fn execute_atom(atom: Atom, gnd: Box<dyn GroundedAtom>, mut args: Vec<Atom>, bindings: Bindings) -> Vec<InterpretedAtom> {
    match gnd.execute(&mut args) {
        Ok(mut results) => {
            if results.is_empty() {
                results.push(return_empty());
            }
            results.into_iter()
...
mod tests {
    ...
    fn interpret_atom_evaluate_atom_no_definition() {
        ...
        assert_eq!(result, vec![atom("(Return Empty)", bind!{})]);
    }
    ...
    fn interpret_atom_evaluate_grounded_expression_noreduce() {
        ...
        assert_eq!(result, vec![InterpretedAtom(expr!("Return" ({NonReducible()} {6})), bind!{})]);
    }
    ...
    fn interpret_atom_evaluate_grounded_expression_error() {
        ...
        assert_eq!(result, vec![InterpretedAtom(expr!("Return" ("Error" ({ThrowError()} {"Test error"}) "Test error")), bind!{})]);
    }
    ...
    fn interpret_atom_chain_return() {
        let result = interpret_atom(&space(""), atom("(chain (Return Empty) $x (bar $x))", bind!{}));
        assert_eq!(result, vec![atom("(bar (Return Empty))", bind!{})]);
    }
